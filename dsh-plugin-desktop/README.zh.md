...
Linux 没有 code signing，三个产物均未签名。CI 会在钉版本的 `ubuntu-24.04` runner 上构建同样的三个产物。AppImage 打包使用 `toolsets.appimage: "1.0.3"` 的静态运行时，因此产物既不会注入 `--no-sandbox`，也不要求用户安装 `libfuse2`。
