...
Packaged macOS, Windows, and Linux applications all check `https://www.dshdesktop.cn/api/desktop/version` in the background. Startup is not blocked; network errors, non-200 responses, invalid versions, and a server version that is not newer remain silent in the background. The Linux build only notifies you that a newer version exists; it has no in-app download.
