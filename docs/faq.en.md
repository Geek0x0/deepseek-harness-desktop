...
Current release installers support Windows x64, Apple Silicon macOS, and Linux x64 (deb, rpm, and AppImage). Intel Macs are not supported, and neither is Linux arm64; cross-platform compatibility code in the source tree does not imply that an installer has been released for that platform. The Linux build only offers compatibility mode, does not include a packaged terminal, and has no in-app update download and no passive update notification; use **Check for Updates…** in the tray to see whether a newer version exists. Minimal desktop environments or container images may be missing `libasound2`, `libgbm1`, `libdrm2`, and `libgl1`; ordinary desktop distributions normally have these preinstalled.
