...
DSH Desktop 是面向 Windows、macOS 和 Linux 的开源 DeepSeek Harness 桌面客户端。它把官方 Harness 的本地 Web UI、Host 服务和插件系统装进原生桌面应用，并提供窗口、系统托盘、终端、更新和 profile 管理。
...
当前正式安装包支持 Windows x64、搭载 Apple 芯片的 macOS，以及 Linux x64（deb、rpm、AppImage）。不支持 Intel Mac，也不支持 Linux arm64；不要根据源码中存在跨平台兼容代码推断已经发布了对应安装包。Linux 版只提供兼容模式，不含打包终端，也没有应用内更新下载，也没有被动的更新提示；需要在托盘手动执行 **Check for Updates…** 才能知道是否有新版本。精简桌面环境或容器镜像可能缺少 `libasound2`、`libgbm1`、`libdrm2`、`libgl1`；普通桌面发行版通常已经预装这些库。
