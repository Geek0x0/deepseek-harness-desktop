...
打包后的 macOS、Windows 和 Linux 应用都会在后台检查 `https://www.dshdesktop.cn/api/desktop/version`。后台检查不阻塞启动；网络错误、非 200、非法版本或服务端版本不新时保持静默。Linux 版没有应用内下载，也没有被动的更新提示，需要在托盘手动执行 **Check for Updates…** 才能知道是否有新版本。
